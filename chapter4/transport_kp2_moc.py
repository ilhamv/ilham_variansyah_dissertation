...
psi_BE_spatial[:,:] = phi0/2

[phi_BE_spatial[1:1+int(N/2)], C_BE_spatial[1:1+int(N/2)], psi_BE_spatial, N_iter, et] = \
    solvers.BE_transport(psi_BE_spatial,C_BE_spatial[0],v,np.ones(J)*SigmaT1,np.ones(J)*nuSigmaF,np.ones(J)*SigmaS1,beta,lam,np.ones(J)*S1,dt,int(N/2),N,X1,J,tol,accelerate,moc)
[phi_BE_spatial[1+int(N/2):], C_BE_spatial[1+int(N/2):], psi_BE_spatial, N_iter, et] = \
    solvers.BE_transport(psi_BE_spatial,C_BE_spatial[int(N/2)],v,np.ones(J)*SigmaT2,np.ones(J)*nuSigmaF,np.ones(J)*SigmaS2,beta,lam,np.ones(J)*S2,dt,int(N/2),N,X2,J,tol,accelerate,moc)
...
psi_MB_spatial[:,:] = phi0/2

[phi_MB_spatial[1:1+int(N/2)], C_MB_spatial[1:1+int(N/2)], psi_MB_spatial, N_iter, et] = \
    solvers.MB_transport(psi_MB_spatial,C_MB_spatial[0],v,np.ones(J)*SigmaT1,np.ones(J)*nuSigmaF,np.ones(J)*SigmaS1,beta,lam,np.ones(J)*S1,dt,int(N/2),N,X1,J,tol,accelerate,moc,store)
[phi_MB_spatial[1+int(N/2):], C_MB_spatial[1+int(N/2):], psi_MB_spatial, N_iter, et] = \
    solvers.MB_transport(psi_MB_spatial,C_MB_spatial[int(N/2)],v,np.ones(J)*SigmaT2,np.ones(J)*nuSigmaF,np.ones(J)*SigmaS2,beta,lam,np.ones(J)*S2,dt,int(N/2),N,X2,J,tol,accelerate,moc,store)
...
for n in range(NN):
    ...
    [phi_BE_spatial[1:1+int(N/2)], C_BE_spatial[1:1+int(N/2)], psi_BE_spatial, N_iter, etBE1] = \
        solvers.BE_transport(psi_BE_spatial,C_BE_spatial[0],v,np.ones(J)*SigmaT1,np.ones(J)*nuSigmaF,np.ones(J)*SigmaS1,beta,lam,np.ones(J)*S1,dt,int(N/2),N,X1,J,tol,accelerate,moc)
    [phi_BE_spatial[1+int(N/2):], C_BE_spatial[1+int(N/2):], psi_BE_spatial, N_iter, etBE2] = \
        solvers.BE_transport(psi_BE_spatial,C_BE_spatial[int(N/2)],v,np.ones(J)*SigmaT2,np.ones(J)*nuSigmaF,np.ones(J)*SigmaS2,beta,lam,np.ones(J)*S2,dt,int(N/2),N,X2,J,tol,accelerate,moc)
    ...
    [phi_MB_spatial[1:1+int(N/2)], C_MB_spatial[1:1+int(N/2)], psi_MB_spatial, N_iter, etMB1] = \
        solvers.MB_transport(psi_MB_spatial,C_MB_spatial[0],v,np.ones(J)*SigmaT1,np.ones(J)*nuSigmaF,np.ones(J)*SigmaS1,beta,lam,np.ones(J)*S1,dt,int(N/2),N,X1,J,tol,accelerate,moc,store)
    [phi_MB_spatial[1+int(N/2):], C_MB_spatial[1+int(N/2):], psi_MB_spatial, N_iter, etMB2] = \
        solvers.MB_transport(psi_MB_spatial,C_MB_spatial[int(N/2)],v,np.ones(J)*SigmaT2,np.ones(J)*nuSigmaF,np.ones(J)*SigmaS2,beta,lam,np.ones(J)*S2,dt,int(N/2),N,X2,J,tol,accelerate,moc,store)
    ...
    np.savetxt(name+"_BE.csv", a, delimiter=",")

    a = np.asarray([ dt_list,err_MB,errS_MB,et_MB ])
    np.savetxt(name+"_MB.csv", a, delimiter=",")