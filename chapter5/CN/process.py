...
for K in K_list:
    ...
    def TT(T):
        ...
        r[0] = (Tf-Tf_prvj)/(dt/2) - (q-qs)/(rho_f(Tf)*Af*cp_f(Tf))
        r[1] = (Tw-Tin)/dz - (qs/cp_w(Tw) - rho_w(Tw)*Aw/(dt/2)*(Tw-Tw_prvj))/mdot
        return r
    ...
    for k in range(K):
        ...
        while err > tol:
            ...
            # Solve neutron
            phi[:], exitCode = gmres(A, 2*phi_prv/(v*dt), x0=phi , M=Pr, tol=tol*1E-2)
