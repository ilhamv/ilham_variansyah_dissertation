...
for K in K_list:
    ...
    D        = np.zeros(J)
    Ddz      = np.zeros(J-1)
    SigmaA_mid   = np.zeros(J)
    nuSigmaF_mid = np.zeros(J)
    SigmaF_mid   = np.zeros(J)
    D_mid        = np.zeros(J)
    Ddz_mid      = np.zeros(J-1)
    ...
    Tf_old   = np.zeros(J)
    Tw_old   = np.zeros(J)
    phi_mid_old  = np.zeros(J)
    Tf_mid_old   = np.zeros(J)
    Tw_mid_old   = np.zeros(J)
    ...
    def TT(T):
        ...
        Tw   = T[3]
        r    = np.zeros_like(T)
        qs_mid = 2*np.pi*Rf*h*(Tf_mid-Tw_mid)
        qs   = 2*np.pi*Rf*h*(Tf-Tw)
        r[0] = (Tf-Tf_prvj)/dt - (q_mid-qs_mid)/(rho_f(Tf_mid)*Af*cp_f(Tf_mid))
        r[1] = (Tw_mid-Tin_mid)/dz - (qs_mid/cp_w(Tw_mid) - rho_w(Tw_mid)*Aw/dt*(Tw-Tw_prvj))/mdot
        r[2] = (Tf-Tf_mid)/dt*2 - (q-qs)/(rho_f(Tf)*Af*cp_f(Tf))
        r[3] = (Tw-Tin)/dz - (qs/cp_w(Tw) - rho_w(Tw)*Aw/dt*2*(Tw-Tw_mid))/mdot
        return r
    ...
    Tf      = np.copy(Tf_ic)
    phi_mid[:] = phi_ic[:]
    Tw_mid     = np.copy(Tw_ic)
    Tf_mid     = np.copy(Tf_ic)
    ...
    for k in range(K):
        ...
        while err > tol:
            ...
            for j in range(J):
                ...
                D[j]        = 1.0/(3*SigmaT)
                SigmaT_mid      = f_SigmaT(Tf_mid[j],Tw_mid[j])
                SigmaS_mid      = f_SigmaS(Tf_mid[j],Tw_mid[j])
                nuSigmaF_mid[j] = f_nuSigmaF(Tf_mid[j],Tw_mid[j])
                SigmaF_mid[j]   = nuSigmaF_mid[j]/nu
                SigmaA_mid[j]   = SigmaT_mid - SigmaS_mid
                D_mid[j]        = 1.0/(3*SigmaT_mid)
            for j in range(J-1):
                Ddz[j] = 2.0/(dz/D[j]+dz/D[j+1])
                Ddz_mid[j] = 2.0/(dz/D_mid[j]+dz/D_mid[j+1])

            # Matrix A: Block 1
            A_ul = -Ddz_mid/dz
            A_d  = SigmaA_mid - nuSigmaF_mid
            for j in range(J-1):
                A_d[j] = A_d[j] - A_ul[j]
            for j in range(1,J):
                A_d[j] = A_d[j] - A_ul[j-1]
            A_d[0]   = A_d[0]   + 2*D_mid[0]/(dz*dz)*(1.0 - 1.0/(0.25*dz/D_mid[0]+1))
            A_d[J-1] = A_d[J-1] + 2*D_mid[J-1]/(dz*dz)*(1.0 - 1.0/(0.25*dz/D_mid[J-1]+1))
            ...
            # Matrix A
            A = sp.sparse.bmat([[A1,A2],[-2*A2,A4]],format="csc")
            ...
            # Solve neutron
            phi_all[:], exitCode = gmres(A, RHS, x0=phi_all, M=Pr, tol=tol*1E-2)

            # Error neutron
            phi_old[:] = phi - phi_old # phi_old stores error
            phi_mid_old[:] = phi_mid - phi_mid_old # phi_old stores error
            err_phi     = np.linalg.norm(abs(np.true_divide(phi_old,phi)))
            err_phi_mid = np.linalg.norm(abs(np.true_divide(phi_mid_old,phi_mid)))
            ...
            for j in range(J):
                ...
                q_mid = kappa_f*SigmaF_mid[j]*phi_mid[j]
                Tf_prvj = Tf_prv[j]
                Tw_prvj = Tw_prv[j]
                [Tf_mid[j],Tw_mid[j],Tf[j],Tw[j]] = sp.optimize.fsolve(TT,[Tf_mid[j],Tw_mid[j],Tf[j],Tw[j]],xtol=tol*1E-2)              
                # Reset Tin
                Tin = Tw[j]
                Tin_mid = Tw_mid[j]
            ...
            Tw_mid_old[:] = Tw_mid - Tw_mid_old
            err_Tw_mid    = np.linalg.norm(abs(np.true_divide(Tw_mid_old,Tw_mid)))
            Tf_mid_old[:] = Tf_mid - Tf_mid_old
            err_Tf_mid    = np.linalg.norm(abs(np.true_divide(Tf_mid_old,Tf_mid)))
            errT_mid      = max(err_Tw_mid,err_Tf_mid)
            errT = max(errT,errT_mid)
