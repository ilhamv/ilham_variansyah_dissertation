...
def BE_infinite(phi_init,C_init,v,SigmaA_list,nuSigmaF,beta,lam,S_list,dt,N,dsource='linear'):
    # Allocate solution
    phi = np.ones(N+1)*phi_init
    C = np.ones(N+1)*C_init
    
    # XS
    nuSigmaFp = (1-beta)*nuSigmaF
    ...
    if dsource == 'BE+':
        ...
        xiC = exp
    if dsource == 'quad':
        xib = 1.0/2/ld*(2/ld*(1-exp)-(1+exp))
        xi0 = 1.0/ld*((1+exp)+(1-2/ld)*(1-exp)) - exp
        xi1 = 1+1.0/2/ld*((2/ld-1)*(1-exp)-2)
        xiC = exp
    for n in range(0,N):
        F        = nuSigmaFp + xi1*nuSigmaFd
        A        = SigmaA_list[n] - F  + 1.0/v/dt
        S        = S_list[n] + (1.0/v/dt + xi0*nuSigmaFd)*phi[n] + xib*nuSigmaFd*phi[n-1] + xiC*lam*C[n]
        phi[n+1] = S/A
        C[n+1]   = xiC*C[n] + nuSigmaFd/lam*(xib*phi[n-1] + xi0*phi[n] + xi1*phi[n+1])
    return phi, C

def CN_infinite(phi_init,C_init,v,SigmaA_list,nuSigmaF,beta,lam,S_list,dt,N):
    # Allocate solution
    phi = np.zeros(N+1)
    C = np.zeros(N+1)
    ...
    for n in range(0,N):
        ...
        M  = np.linalg.inv(M)
        Q    = np.array([[v*S],[0]])
        Phi  = M.dot((2*I/dt-A).dot(Phi) + 2*Q)
        phi[n+1] = Phi[0]
        C[n+1]   = Phi[1]
    return phi, C

def MB_infinite(phi_init,C_init,v,SigmaA_list,nuSigmaF,beta,lam,S_list,dt,N,dsource='standard'):
    # Allocate solution
    phi = np.zeros(N+1)
    C = np.zeros(N+1)
    ...
    if dsource == "standard":
        xi0_1     = 0.0
        xi12_1    = ld/(1+ld+ld**2/2)
        xi1_1     = ld**2/2/(1+ld+ld**2/2)
        xi0_12    = 0.0
        xi12_12   = 1-1/(1+ld+ld**2/2)
        xi1_12    = -ld/2/(1+ld+ld**2/2)
        xiC_1     = 1/(1+ld+ld**2/2)
        xiC_12    = (1-1/(1+ld+ld**2/2))/ld

    if dsource == "quad":
        xi0_1     = -exp - (1+3*exp)/ld + 4*(1-exp)/ld**2
        xi12_1    = 4*(1+exp)/ld - 8*(1-exp)/ld**2
        xi1_1     = 1 - (3+exp)/ld + 4*(1-exp)/ld**2
        xi0_12    = -exp2 + (1-3*exp2)/ld + 4*(1-exp2)/ld**2
        xi12_12   = 1 + 4*exp2/ld - 8*(1-exp2)/ld**2
        xi1_12    = -(1+exp2)/ld + 4*(1-exp2)/ld**2
        xiC_1     = exp
        xiC_12    = exp2
    ...
    if dsource == "linear":
        ...
        xi12_1    = 0.0
        xi1_1     = 1 - (1-exp)/ld
        xi0_12    = 0.5 + (1-exp2)/ld - exp2
        xi12_12   = 0.0
        xi1_12    = 0.5 - (1-exp2)/ld
        xiC_1     = exp
        xiC_12    = exp2
    ...
    for n in range(0,N):
        SigmaA = SigmaA_list[n]
        S      = S_list[n]
        A12_12 = SigmaA - nuSigmaFp - xi12_12*nuSigmaFd
        A1_12  = -xi1_12*nuSigmaFd
        A12_1  = xi12_1*nuSigmaFd
        A1_1   = SigmaA - nuSigmaFp - xi1_1*nuSigmaFd
        Sf1    = S + xi0_1*nuSigmaFd*phi[n]+xiC_1*lam*C[n]
        Sf12   = S + (xi0_12*nuSigmaFd+1/dt/v)*phi[n]+xiC_12*lam*C[n]
        Sf     = Sf12 + A12_12/(A12_1+2/v/dt)*Sf1
        phi[n+1] = Sf/(A12_12*(A1_1+2/v/dt)/(A12_1+2/v/dt)+(A1_12+1/v/dt))
        phi_mid  = (A1_1+2/v/dt)/(A12_1+2/v/dt)*phi[n+1]-Sf1/(A12_1+2/v/dt)
        C[n+1]   = xiC_1*C[n] + 1.0/lam*nuSigmaFd*(xi0_1*phi[n]+xi12_1*phi_mid+xi1_1*phi[n+1])
    return phi, C

# =============================================================================
# Numerical solution (Diffusion)
# =============================================================================

def BE_diffusion(phi_init,C_init,v,SigmaA,nuSigmaF,D,beta,lam,Q,dt,N,X,J,aR,aL,tol):
    # Time!
...
def MB_diffusion(phi_init,C_init,v,SigmaA,nuSigmaF,D,beta,lam,Q,dt,N,X,J,aR,aL,tol):
    # Time!
    time_start = time.time()
...
def MB_diffusion_O(phi_init,C_init,v,SigmaA,nuSigmaF,D,beta,lam,Q,dt,N,X,J,aR,aL,tol):
    # Time!
    time_start = time.time()
    ...
    # Block A1
    A_ul     = -SigmaJ[1:-1]
    A_d      = SigmaA - (nuSigmaFp+xi12_12*nuSigmaFd) + 1.0/v/dt + SigmaJ[:-1] + SigmaJ[1:]
    A_d[0]  -= SigmaJ[0]*BL
    A_d[-1] -= SigmaJ[-1]*BR
    A1 = diags([A_ul,A_d,A_ul],[-1,0,1],format="csc")
    ...
    # Block A2
    A2  = 1/(v*dt) - xi1_12*nuSigmaFd
    
    # Block A3
    A3 = -2/(v*dt) - xi12_1*nuSigmaFd
     
    # Create Pr
    Pr1 = spla.spilu(A1)
    Pr1 = spla.LinearOperator(A1.shape, Pr1.solve)
    Pr4 = spla.spilu(A4)
    Pr4 = spla.LinearOperator(A4.shape, Pr4.solve)
    ...
    for n in range(0,N):        
        ...
        while err > tol:
            ...
            # Set RHS
            b[:] = Q + phi_prv/v/dt + xiC_12*lam*C - A2*phi_old[J:] + phi_old[:J]/v/dt
            # Solve phi12
            phi[:J], exitCode = gmres(A1, b, x0=phi[:J], M=Pr1, tol=tol*1E-2)
            ...
            # Set RHS
            b[:] = Q + xiC_1*lam*C - A3*phi[:J]
            # Solve phi12
            phi[J:], exitCode = gmres(A4, b, x0=phi[J:], M=Pr4, tol=tol*1E-2)
            
            # Error
            err = np.linalg.norm(np.divide(phi-phi_old,phi))
...
def MB_diffusion_subs(phi_init,C_init,v,SigmaA,nuSigmaF,D,beta,lam,S,dt,N,X,J,aR,aL,tol):
    # Time!
    time_start = time.time()
    ...
    # Block A2
    A_d = np.ones(J)/(v*dt) - xi1_12*nuSigmaFd
    A2  = diags([A_d],[0],format="csc")
    
    # Block A3
    A3 = 1/(2/(v*dt) + xi12_1*nuSigmaFd[0])
    ...
    # March in time        
    for n in range(0,N):
        SS[:] = Q + phi/v/dt + xiC_12*lam*C + A1.dot(xiC_1*lam*C)*A3
        phi[:], exitCode = gmres(A, SS, x0=phi, M=Pr, tol=tol*1E-2)
        phi_mid = (A4.dot(phi) - S - xiC_1*lam*C)*A3
        C = xiC_1*C + (xi12_1*phi_mid+xi1_1*phi)*nuSigmaFd/lam
        phi_t[n][:] = phi[:]
        C_t[n][:]   = C[:]
...
def BE_transport(psi_init,C_init,v,SigmaT,nuSigmaF,SigmaS,beta,lam,Q,dt,K,N,X,J,eps,accelerate,moc):
    # Time!
    time_start = time.time()
    ...
    def run_sweep(phi_old,S,phi,psi_avg,phi_prv,C,update=False):
        # Reset phi
        phi_old[:] = phi[:]
        phi[:] = 0.0
        ...
        # Set source
        for j in range(J):
            xi = (SigmaS[j]+nuSigmaFp[j]+xi1*nuSigmaFd[j])/SigmaT[j]
            S[j] = 0.5*(xi*phi_old[j] + (Q[j] + xi0*nuSigmaFd[j]*phi_prv[j] + xiC*lam*C[j])/SigmaT[j])
    ...
    def dsa(phi,phi_old,F,Q,tol):
        ...
        Q[:] = (phi-phi_old)*(SigmaS+nuSigmaFp+xi1*nuSigmaFd)
    ...
    for k in range(K):
        ...
        while err > tol:
            # Sweep!
            run_sweep(phi_old,S,phi,psi_avg,phi_prv,C)
            N_iter = N_iter + 1
            ...
            tol = (1-rho)*eps
            #print("BE    ",2*K,k,err,rho)

        # Final sweep to update psi_avg
        run_sweep(phi_old,S,phi,psi_avg,phi_prv,C,True)
        N_iter = N_iter + 1
        
        # Store
        for j in range(J):
            C[j] = xiC*C[j] + nuSigmaFd[j]/lam*(xi0*phi_prv[j]+xi1*phi[j])
        phi_t[k][:] = phi[:]
        C_t[k][:] = C[:]

    # Time!
    time_end = time.time()
    return phi_t, C_t, psi_avg, N_iter, time_end - time_start

def MB_transport(psi_init,C_init,v,SigmaT,nuSigmaF,SigmaS,beta,lam,Q,dt,K,N,X,J,eps,accelerate,moc,store):
    # Time!
    time_start = time.time()
    ...
    def run_sweep(phi1_old,phi2_old,S1,S2,phi1,phi2,psi_avg,C,update=False):
        # Reset phi
        phi1_old[:] = phi1[:]
        phi2_old[:] = phi2[:]
        phi1[:] = 0.0
        ...
        # Set source
        for j in range(J):
            xi = (SigmaS[j]+nuSigmaFp[j])
            S1[j] = 0.5*((xi+xi12_12*nuSigmaFd[j])*phi1_old[j] + xi1_12*nuSigmaFd[j]*phi2_old[j] + Q[j] + lam*xiC_12*C[j])/SigmaT[j]
            S2[j] = 0.5*((xi+xi1_1*nuSigmaFd[j])*phi2_old[j] + xi12_1*nuSigmaFd[j]*phi1_old[j] + Q[j] + lam*xiC_1*C[j])/SigmaT[j]
    ...
    def dsa(phi1,phi2,phi1_old,phi2_old,F,Q,tol):
        # Set error/correction source
        Q[:J] = (phi1 - phi1_old)*(SigmaS+nuSigmaFp+xi12_12*nuSigmaFd) + (phi2 - phi2_old)*xi1_12*nuSigmaFd
        Q[J:] = (phi2 - phi2_old)*(SigmaS+nuSigmaFp+xi1_1*nuSigmaFd) + (phi1 - phi1_old)*xi12_1*nuSigmaFd
    ...
    for k in range(K):
        ...
        while err > tol:
            # Sweep!
            run_sweep(phi1_old,phi2_old,S1,S2,phi1,phi2,psi_avg,C)
            N_iter = N_iter + 1
            ...
            # Tolerance considering false convergence
            tol = (1-rho)*eps
            #print("MB    ",2*K,k,err,rho)


        # Final sweep to update psi_avg
        run_sweep(phi1_old,phi2_old,S1,S2,phi1,phi2,psi_avg,C,True)
        N_iter = N_iter + 1
        
        # Store
        for j in range(J):
            C[j] = xiC_1*C[j] + nuSigmaFd[j]/lam*(xi12_1*phi1[j]+xi1_1*phi2[j])
        phi_t[k][:] = phi2[:]
        C_t[k][:] = C[:]
